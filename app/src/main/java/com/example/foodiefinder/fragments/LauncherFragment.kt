...
class LauncherFragment : Fragment() {
    ...
    override fun onStart() {
        ...
        if (currentUser != null && Util.isNetworkAvailable(requireActivity())) {
        ...
        auth.signInWithCredential(credential)
            .addOnCompleteListener(requireActivity()) { task ->
                if (task.isSuccessful) {
                    val currentUser = auth.currentUser
                    Toast.makeText(requireActivity(), "Signed in as ${currentUser?.displayName}", Toast.LENGTH_SHORT)
                        .show()
                    val intent = Intent(requireActivity(), MainActivity::class.java)
                    startActivity(intent)
                    requireActivity().finish()
                } else {
                    Toast.makeText(requireActivity(), "Hmm, something went wrong. Please try again", Toast.LENGTH_SHORT).show()
                }
            }
    }
}